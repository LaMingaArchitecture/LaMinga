...
4. **À récupérer** : **Preview token** (preview SSR) et **Public token** (prod SSG) — _Settings → Access Tokens_. Les valeurs réelles sont stockées dans le gestionnaire de mots de passe partagé (jamais dans le dépôt) et injectées via `.env` en local / variables d'environnement Netlify par contexte.
...
2. Prévoir **deux contextes** : _production_ (branche `main`, SSG) et _branch deploy_ `preview` (SSR).
